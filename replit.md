...
- **PayHero M-Pesa STK Push.** `api-server/src/routes/api/payments/payhero.ts` (`/api/payments/payhero/{stk,status,callback}`). Env: `PAYHERO_BASIC_AUTH_TOKEN` (or `PAYHERO_API_USERNAME`+`PAYHERO_API_PASSWORD`), `PAYHERO_CHANNEL_ID`, `PAYHERO_CALLBACK_URL`. Storefront flow: `MpesaPaymentModal` from `checkout-page.tsx`.
- **Paystack** (`/api/v2/payments/paystack/*`): KES `mobile_money.phone` MUST be E.164 with leading `+`. Never grant value on a client-supplied reference — verify server-side and bind to the order.
