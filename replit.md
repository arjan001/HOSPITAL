...
- **NestJS user backend:** `artifacts/api-nest/` — mounted at `/api/v2`. Cookie-based guest session today (Clerk-ready). **All modules are now Postgres-durable via Drizzle (`@workspace/db`)** — profile, addresses, wishlist, orders, prescriptions, notifications, payments, uploads, patient-notes, partner submissions, and CMS all survive restarts/deploys. The session→user bridge is `common/session-user.ts` (`ensureUserId(sid)` upserts a `users` row with `clerkId = req.sessionId`); every per-session entity resolves a `userId` from it. When Clerk lands, swap the session middleware to set `req.sessionId = clerkUserId` — services don't change.
...
- **NestJS user backend (May 2026).** New artifact `api-nest` runs at `/api/v2` alongside the legacy Express server. Strangler migration: customer + admin modules (profile, addresses, wishlist, orders, prescriptions, notifications, payments, uploads, patient-notes, partners, CMS) are served by NestJS and **fully Postgres-durable**; legacy `/api` keeps backing the storefront catalog/banners until those port over. Session is Clerk-pending (see the dedicated section below).
- **Supabase has been fully removed (May 2026).** All admin-managed content lives in `cmsStore`, now backed by the api-nest `admin-cms` module → Postgres `cms_docs` (localStorage is a client cache only; the DB is the source of truth). The legacy `/api/admin/*` and public `/api/*` routes still exist but back onto a no-op stub in `artifacts/api-server/src/lib/legacy-store.ts` that returns empty reads / soft-error writes — they're scheduled to be deleted as the NestJS port lands. Do **not** add new code that imports from the supabase stubs; persist via `cmsStore` instead.
...
- **Persistence.** Every service is Drizzle-backed against `@workspace/db` (`lib/db`). Per-session entities resolve a `userId` via the `session-user.ts` bridge (`users.clerkId = req.sessionId`); admin cross-session reads resolve the owner back through `users.clerkId`. Services are `async` (like chat) but their public method signatures are unchanged, so controllers + the storefront client are untouched. Any new persisted field MUST land in `packages/db` in the same change (see DB-schema discipline below).
...
- **Modules shipped (all Postgres-durable):** `health`, `profile` (`/me`), `addresses` (`/me/addresses`), `wishlist` (`/me/wishlist`), `orders` (`/me/orders`), `prescriptions` (`/me/prescriptions` + `/admin/prescriptions`), `paystack` (`/payments/paystack/{charge,status,callback}`), `admin-payments`, `admin-orders`, `notifications`, `uploads`, `patient-notes` (`/admin/patients/:id/notes`), `partners` (`/partners/:type/*`), `admin-cms` (cms_docs), `whatsapp` (`/notifications/whatsapp/{status,send}`), `error-reporting` (`/admin/error-reporting/{status,test}`, `@Global`), `storage` (`/admin/storage/{status,test}`), `chat` (`/chat/me/*`, `/chat/admin/*` — see Live chat below). Cart is still pending. Partner submissions persist through the durable `cmsStore` mirror (cms_docs) — `cmsPut` is **fail-closed** (a dropped mirror write surfaces as an error to the caller instead of a silent loss).

### Live chat (`/api/v2/chat`, realtime via SSE)

WhatsApp-style consultation chat between patients and the pharmacy team. **Realtime transport is Server-Sent Events — no new env, no extra service.** Lives in `artifacts/api-nest/src/modules/chat.module.ts`; threads/messages are **Postgres-durable via Drizzle** (`@workspace/db` → `chat_threads` / `chat_messages`), keyed by `req.sessionId`. Only the SSE subjects + online/typing presence stay in memory (runtime transport state that is meaningless after a restart). Controllers are unchanged — `ChatService` methods are now `async` (return `Promise`).

- **Chat survives server restarts and deploys.** Threads, messages, attachments, and read/delivery state (`status`, `deliveredAt`, `readAt`) all live in Postgres, keyed by the signed `shaniidrx_sid` session cookie. Presence (online/last-seen) is in-memory by design and resets on restart — the conversation itself does not.
- **End-of-consultation = saved transcript (May 2026).** A thread carries `status` (`active`|`archived`) + `closedAt`, and an optional `consultationId` FK. Ending a consultation archives the thread and **keeps every message** (it is never deleted) so the transcript is a retrievable saved record; a new message reopens an archived thread (`status`→`active`). Endpoints: patient `POST /chat/me/close`, admin `POST /chat/admin/threads/:id/close` (both accept optional `{ consultationId }`). Wired from: patient dashboard `account/chat.tsx` (SessionTimer `onEnd`), the `speak-to-a-doctor.tsx` funnel (`endConsultation` → both the chat "End Consultation" button and the videocall `onLeave`), and the doctor inbox `admin/chat.tsx` ("End & save" button + a "Saved" badge when archived). Messages also carry `deliveredAt`/`readAt` timestamps. Persisted in Postgres via `lib/db/src/schema/chat.ts` (thread↔consultation relation in `relations.ts`); archived threads are durable transcripts that survive restarts/deploys.
...
**Any change that adds or alters persisted data MUST land the matching schema in `packages/db` (`lib/db`) in the same change.** Every api-nest module is now Drizzle-backed, so:
- New entity/field → add/extend the Drizzle table + Zod schema in `packages/db` first, then use it from the service.
...
- Keep the runtime shape and `packages/db` identical; they are the single source of truth for production Postgres.
