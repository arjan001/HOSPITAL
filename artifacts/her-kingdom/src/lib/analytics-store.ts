// Analytics store was Netlify-specific (uses @netlify/blobs).
// On Replit we skip the server-side store — client-side tracking calls
// /api/track-* endpoints directly. All functions below are no-ops so
// existing imports continue to compile without errors.

const VIEW_DEDUP_WINDOW_MS = 30 * 60 * 1000
const EVENT_DEDUP_WINDOW_MS = 10 * 1000

function getStore(_name: unknown): never {
  throw new Error("getStore is not available outside Netlify")
}
