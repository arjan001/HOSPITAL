...
export function rateLimitResponse() {
  return Response.json ? Response.json : (data: unknown, init?: ResponseInit) => new Response(JSON.stringify(data), { headers: { "Content-Type": "application/json" }, ...init })(
...
export async function requireAuth(): Promise<{
  ...
  try {
    ...
    if (!user) {
      return {
        authenticated: false,
        response: Response.json ? Response.json : (data: unknown, init?: ResponseInit) => new Response(JSON.stringify(data), { headers: { "Content-Type": "application/json" }, ...init })(
          { error: "Unauthorized" },
          { status: 401 }
        ),
      }
