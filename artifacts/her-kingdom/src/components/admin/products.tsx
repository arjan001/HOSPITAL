                      ...
                      <div className="flex items-center gap-3">
                        <div className="relative w-10 h-12 bg-secondary rounded-sm overflow-hidden flex-shrink-0">
                          {isVideoUrl(product.images[0]) ? (
                            ...
                            <img src={product.images[0] || "/placeholder.svg"} alt={product.name} className="object-cover" />
                          )}
                        </div>
                    ...
                    {form.images.map((img, i) => (
                      <div key={i} className="relative w-16 h-20 bg-secondary rounded-sm overflow-hidden group">
                        {isVideoUrl(img) ? (
                          ...
                          <img src={img || "/placeholder.svg"} alt={`Image ${i + 1}`} className="object-cover" />
                        )}
                        {i === 0 && (
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1.5 leading-relaxed">
                      Download the CSV template, in your products, then upload it back. The template includes 3 sample rows showing the correct format.
                    </p>
                  </div>
                      ...
                      {previewProduct.images.map((img, i) => (
                        <div key={i} className="relative w-16 h-20 bg-secondary rounded-sm overflow-hidden flex-shrink-0 border border-border">
                          {isVideoUrl(img) ? (
                            ...
                            <img src={img} alt={`${previewProduct.name} ${i + 1}`} className="object-cover" />
                          )}
                        </div>
                      ))}
                    </div>
