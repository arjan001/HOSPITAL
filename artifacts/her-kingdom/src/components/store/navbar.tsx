                ...
                <div className="absolute top-full left-0 right-0 mt-1 bg-background border border-border shadow-lg rounded-sm z-50 overflow-hidden">
                  {suggestions.length > 0 ? (
                    ...
                    <div className="px-4 py-6 text-center">
                      ...
                      <button type="button" onClick={() => { navigate(`/search?q=${encodeURIComponent(searchQuery.trim())}`); setShowSuggestions(false); setSearchQuery("") }} className="text-xs underline mt-2 text-muted-foreground hover:text-foreground">
                        Search the shop
                      </button>
                    </div>
                  )}
