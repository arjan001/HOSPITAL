...
export function CheckoutPage() {
  ...
  const handlePaystackConfirmed = (result: { orderNumber: string; mpesaReceipt: string; phone: string; reference: string; method: "mpesa" | "card" }) => {
    ...
    if (fulfilmentMode === "delivery") {
      void (async () => {
        try {
          const allPartners = await fetchActiveLogisticsPartners()
          const county = (savedAddress?.region || "").toLowerCase()
          const active = allPartners.filter(p => p.status === "active" || p.status === "verified")
  ...
  if (orderResult) {
    ...
    const deliveryLine = orderResult.fulfilmentMode === "pickup"
      ? `Ready for pickup${orderResult.locationLabel ? ` at ${orderResult.locationLabel}` : ""}.`
      ...
      <div className="min-h-screen flex flex-col" style={{ background: CREAM }}>
        ...
        <main className="flex-1 py-12 px-4">
          <div className="max-w-lg mx-auto text-center">
            ...
            {/* Peach circle + wine check */}
            <div
              className="w-20 h-20 mx-auto mb-6 rounded-full flex items-center justify-center"
              style={{ background: PEACH_LIGHT }}
            >
              <CheckCircle className="h-10 w-10" style={{ color: WINE }} strokeWidth={2.25} />
            </div>

            {/* Headline */}
            <h1 className="text-2xl sm:text-3xl font-bold mb-3" style={{ color: WINE }}>
              {heading}
            </h1>
            ...
            {!isWhatsApp && (
              <div
                className="inline-flex items-center gap-2 px-4 py-1.5 rounded-md mb-8"
                style={{ background: PEACH_MED }}
              >
                <span className="text-xs font-medium" style={{ color: WINE }}>Order No:</span>
                <span className="font-mono text-sm font-bold tracking-wider" style={{ color: WINE }}>
                  {orderResult.orderNumber}
                </span>
              </div>
            )}

            {/* Payment received callout (peach bg, wine left border) */}
            <div
              className="text-left rounded-md p-4 mb-3 flex items-start gap-3"
              style={{ background: PEACH_LIGHT, borderLeft: `3px solid ${WINE}` }}
            >
              <div
                className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5"
                style={{ background: "#fff" }}
              >
                <CheckCircle className="h-4 w-4" style={{ color: WINE }} />
              </div>
              <div className="min-w-0">
                <p className="text-sm font-bold mb-0.5" style={{ color: WINE }}>{paymentTitle}</p>
                <p className="text-xs leading-relaxed" style={{ color: "#7c4a3a" }}>{paymentBody}</p>
                {isMpesa && orderResult.mpesaReceipt && (
                  <p className="text-xs mt-1.5">
                    <span className="text-[10px] uppercase tracking-wider" style={{ color: "#7c4a3a" }}>Ref</span>
                    <span className="ml-2 font-mono font-bold" style={{ color: WINE }}>{orderResult.mpesaReceipt}</span>
                  </p>
                )}
              </div>
            </div>

            {/* Delivery callout (subtle) */}
            <div
              className="text-left rounded-md p-4 mb-8 flex items-center gap-3"
              style={{ background: "#FBF4EA", border: "1px solid #F2DCC8" }}
            >
              <Package className="h-4 w-4 flex-shrink-0" style={{ color: WINE }} />
              <p className="text-sm" style={{ color: "#7c4a3a" }}>{deliveryLine}</p>
            </div>
            ...
            <div className="flex flex-col gap-3">
              <button
                ...
                className="w-full h-12 rounded-md font-bold text-white flex items-center justify-center gap-2 transition-opacity hover:opacity-90 text-sm"
              ...
              <Link href="/shop" className="w-full">
                <button
                  className="w-full h-12 rounded-md font-semibold text-sm bg-white hover:bg-neutral-50 transition-colors"
                  style={{ color: WINE, border: `1px solid ${PEACH_MED}` }}
                >
                  Continue Shopping
                </button>
              </Link>
