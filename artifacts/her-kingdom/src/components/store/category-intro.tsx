...
export function CategoryIntro({
  ...
  const description =
    ...
    `Shop ${category.name.toLowerCase()} at Shaniid RX Nairobi. Authentic, quality-checked ${category.name.toLowerCase()} delivered across Kenya. Same-day Nairobi delivery, nationwide courier and WhatsApp ordering.`
      ...
      <div>
        ...
        <p className="mt-2 text-xs text-muted-foreground">
          {products.length > 0
            ...
            : `New ${category.name.toLowerCase()} arriving soon. Browse the full Shaniid RX catalog or message us on WhatsApp ${phoneDisplay} for personal recommendations.`}
        </p>
        ...
        {topProducts.length === 0 && (
          <div className="mt-6">
            ...
            >
              Browse the full Shaniid RX catalog
            </Link>
          </div>
        )}
      </div>
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              "@type": "ItemList",
              name: `${category.name} at Shaniid RX`,
