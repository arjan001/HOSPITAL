...
import { db, communicationOutbox, communicationSentLog, campaignSends, sourcingCompetitorPrices, sourcingPriceHistory } from "@workspace/db"
...
import { SourcingExtModule, SourcingAutomationDbService } from "./sourcing-ext.module"
...
class TradingAutomationService {
  ...
  async recomputeMargins(targetMarginPct = 25): Promise<{
    ...
    const [historyRows, competitorRows] = await Promise.all([
      db.select().from(sourcingPriceHistory).orderBy(desc(sourcingPriceHistory.capturedAt)),
      db.select().from(sourcingCompetitorPrices),
    ])
    ...
    const competitor = competitorRows.map((row) => ({
      sku: row.sku,
